...
using static RawInput_dll.KeyPressEvent;
...
namespace RawInput_dll
{
	public sealed class RawKeyboard
	{
		...
		public void ProcessRawInput(IntPtr hdevice)
		{
			...
			keyPressEvent.Message = (MessagesEnum)_rawBuffer.data.keyboard.Message;
			keyPressEvent.VKeyName = KeyMapper.GetMicrosoftKeyName(virtualKey).ToUpper();
			keyPressEvent.VKey = virtualKey;// VirtualKeyCorrection(virtualKey, isE0BitSet, makeCode);
			keyPressEvent.RawVKey = virtualKey;


			if (KeyPressed != null)
			{
				var eventObj = new RawInputEventArg(keyPressEvent);
				KeyPressed(this, eventObj);
			}
		}
